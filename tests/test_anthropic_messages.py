...
from echoflow.impl.anthropic.messages import AnthropicStaticMessages, AnthropicDynamicMessages, AnthropicAdapter
...
class TestAnthropicAdapter(unittest.TestCase):
    ...
    def test_to_message(self):
        ...
        element = {'role': 'user', 'content': [{'type': 'tool_result', 'tool_use_id': '12345', 'content': '54321'}]}
        expected = Message(role="tool", content=[ToolResult(id="12345", content="54321")])
        self.assertEqual(self.adaptor.to_message(element), expected)

        element = {'role': 'assistant',
                   'content': [{'type': 'tool_use', 'id': '12345', 'name': 'random', 'input': {'args': '12345'}}]}
        expected = Message(role="assistant", content=[ToolCall(id="12345", name="random", input={"args": "12345"})])
        self.assertEqual(self.adaptor.to_message(element), expected)
...
class TestAnthropicDynamicMessages(unittest.TestCase):
    def setUp(self):
        ...
        messages.add_message(Message(role="assistant", content=[
            "second reply", ToolCall(id="12345", name="random", input={"args": "12345"})]))
        messages.add_message(Message(role="tool", content=[ToolResult(id="12345", content="54321")]))
    ...
    def test_message(self):
        expected = [
            ...
            Message(role='assistant',
                    content=['second reply', ToolCall(id='12345', name='random', input={'args': '12345'})]),
    ...
    def test_message_value(self):
        expected = [
            {'role': 'user',
             'content': [{'text': '1st request', 'type': 'text'}, {'text': 'second request', 'type': 'text'}]},
            {'role': 'assistant',
             'content': [{'text': 'first reply', 'type': 'text'}, {'text': 'second reply', 'type': 'text'},
                         {'type': 'tool_use', 'id': '12345', 'name': 'random', 'input': {'args': '12345'}}]},
            {'role': 'user', 'content': [{'type': 'tool_result', 'tool_use_id': '12345', 'content': '54321'}]},
...
class TestAnthropicStaticMessages(unittest.TestCase):
    ...
    def test_message(self):
        expected = [
            ...
            Message(role='assistant', content=['first reply', 'second reply',
    ...
    def test_message_value(self):
        expected = [
            {'role': 'user',
             'content': [{'text': 'first request', 'type': 'text'}, {'text': 'second request', 'type': 'text'}]},
            {'role': 'assistant',
             'content': [{'text': 'first reply', 'type': 'text'}, {'text': 'second reply', 'type': 'text'},
                         {'type': 'tool_use', 'id': '12345', 'name': 'random', 'input': {'args': '12345'}}]},
            {'role': 'user', 'content': [{'type': 'tool_result', 'tool_use_id': '12345', 'content': '54321'}]},
